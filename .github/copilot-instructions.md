...
- **生态文档（Ecosystem）**：面向交付/适配/运维，沉淀华为、海光与开源社区的驱动、镜像、适配与排障文档。
...
- `src/pages/docs/cn/<section>/toc.ts`：各个产品（Rune/Boss/Moha）的侧边栏项迁移到各自目录的 `toc.ts`，便于按产品拆分维护。
- `src/pages/docs/cn/40.ecosystem/toc.ts`：生态文档侧边栏配置与信息架构。
...
## 最近历史变更
- 顶部导航当前包含 `首页`、`魔哈仓库`、`Rune 智算平台`、`Boss 运营平台`、`生态文档`；去除版本切换和 F&Q/About 入口，版本信息由具体文档页内部控制。
...
- 新增 `生态文档`：侧边栏目录独立（华为/海光/开源社区），并在生态文档页隐藏侧边栏顶部的平台标题区块，避免展示与生态无关的站点名称。
...
- `src/pages/docs/cn/30.moha` 下各个子目录（如 `01.models`、`02.datasets` 等）已包含和侧边栏对应的占位 Markdown 页面，添加/重命名页面时务必与 `30.moha/toc.ts` 中的 `path` 字段保持一致。
- 占位页面可使用统一 frontmatter 模板（title/updated/author/description/tags）并尽量补全描述段落，后续再丰富内容即可。
