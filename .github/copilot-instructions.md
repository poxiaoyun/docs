...
## 最近历史变更
- 顶部导航精简为仅保留 `首页`、`魔哈仓库`、`Rune`、`BOSS` 四个产品项，去除版本切换和 F&Q/About 入口，并在相同组件中先行展示云平台名；版本信息由具体文档页内部控制。
...
- 首页简化设计：首页组件（`src/sections/home/hero.tsx`）简化为仅包含标题、副标题和三个产品卡片（魔哈仓库、Rune、BOSS），卡片可点击跳转到对应文档页面；`HomeView` 已简化为仅包含 `HomeHeroSection`。
*** End Patch
