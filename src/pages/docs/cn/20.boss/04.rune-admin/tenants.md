...
与 [IAM 租户管理](../iam/tenants) 不同，Rune 租户管理聚焦于**计算资源层面**——管理员在此为租户分配 GPU、CPU、内存等配额，管理工作空间和算力规格，确保资源在多租户之间合理分配。

:::tip
Rune 租户列表复用了 IAM 租户列表数据，并在此基础上扩展了 **TenantQuota**（租户配额）列，直观展示各租户在每个集群中的资源配额分配情况。
:::
...
![租户资源列表](/assets/screenshots/boss/rune-tenants.png)
...
:::tip
配额列中的数值格式为「已使用 / 总配额」，便于管理员快速识别各租户的资源使用率。
:::
...
![租户概览](/assets/screenshots/boss/rune-tenant-overview.png)
...
![配额管理](/assets/screenshots/boss/rune-tenant-quotas.png)
...
- **规格筛选栏**：按算力规格（Flavor）筛选配额，快速定位特定资源类型的配额
...
:::warning
降低配额上限时，如果当前使用量已超过新的上限，现有工作负载不会受影响，但租户将无法申请新的资源，直到使用量降至配额以下。
:::
...
:::warning
删除配额后，租户在该集群中将无法使用对应的算力规格，可能导致现有工作负载调度失败。
:::
...
![工作空间管理](/assets/screenshots/boss/rune-tenant-workspaces.png)
...
:::warning
删除工作空间将同时删除该命名空间下的所有资源（应用实例、任务、数据等），此操作不可撤销。
:::
...
![算力规格管理](/assets/screenshots/boss/rune-tenant-flavors.png)

租户级别的算力规格管理，控制租户可以使用哪些算力规格（Flavor）。

:::tip
此处管理的是租户**可见**的算力规格列表。只有在此处启用的 Flavor，租户用户才能在创建应用时选择。全局算力规格的管理请参见 [算力规格管理](./flavors)。
:::
...
1. 在 [IAM 租户管理](../iam/tenants) 中创建租户
