...
![BOSS 仪表盘](/assets/screenshots/boss/dashboard.png)
...
:::tip
当前仪表盘数据为 **（示例数据）**（模拟数据），用于展示仪表盘的布局和交互设计。后续版本将接入真实后端 API，届时所有指标将实时更新。
:::
...
![Moha 资源卡片](/assets/screenshots/boss/dashboard-moha-card.png)
...
:::tip
如果私有资源占比过高，可能意味着平台的资源共享文化有待加强；反之如果公开资源过多，则需关注数据安全合规。
:::
...
![LLM 网关运营卡片](/assets/screenshots/boss/dashboard-gateway-card.png)
...
:::warning
网关卡片目前使用 （示例数据）。接入真实数据后，KPI 指标和趋势图将支持自定义时间范围的筛选。
:::
...
![平台管理卡片](/assets/screenshots/boss/dashboard-management-card.png)
...
:::tip
待处理事项可点击直接跳转到对应的管理页面，帮助管理员快速响应。
:::
...
flowchart TB
    ...
    subgraph Mock["当前状态"]
        MockData["⚠️ 使用 （示例数据）<br/>后续版本接入真实 API"]
    end
...
:::warning
即使仪表盘显示一切正常，也建议定期进入各管理模块进行深入检查，仪表盘展示的是概要信息，无法覆盖所有细节。
:::
