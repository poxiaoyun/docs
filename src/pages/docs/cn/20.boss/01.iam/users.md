...
![用户管理列表](/assets/screenshots/boss/iam-users.png)
...
:::tip
搜索支持实时过滤，输入关键词后列表会自动刷新匹配结果，无需手动点击搜索按钮。
:::
...
![创建用户表单](/assets/screenshots/boss/iam-users-create.png)
...
:::warning
创建用户时**不需要手动设置密码**。系统将自动生成一个随机强密码，并在创建成功后通过弹窗展示。
:::
...
![密码生成弹窗](/assets/screenshots/boss/iam-users-password-dialog.png)
...
:::tip
自动生成的密码仅在创建时展示一次。如果管理员关闭弹窗后忘记密码，只能通过「重置密码」功能重新生成。
:::
...
![编辑用户表单](/assets/screenshots/boss/iam-users-edit.png)
...
:::tip
编辑用户信息不会影响用户的密码和 MFA 设置。密码修改请使用「重置密码」功能。
:::
...
:::warning
密码重置操作不可撤销。重置后旧密码将立即失效，请确保已将新密码安全传达给用户。
:::
...
![删除用户确认](/assets/screenshots/boss/iam-users-delete-confirm.png)
...
:::tip
MFA 状态由用户自行在 Console 的个人安全设置中管理。管理员在 BOSS 中只能查看 MFA 状态，不能代替用户开启或关闭 MFA。出于安全考虑，建议管理员通过公告或通知鼓励所有用户启用 MFA。
:::
