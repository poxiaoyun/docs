...
:::tip
内容审查功能通过 [网关配置](./config) 中的 `moderationEnabled` 开关控制全局启用/关闭。关闭后所有审查策略将暂停执行，但策略配置不会丢失。
:::
...
![审查策略列表](/assets/screenshots/boss/gateway-moderation.png)
...
![创建审查策略](/assets/screenshots/boss/gateway-moderation-policy-create.png)
...
:::tip
建议采用分级策略——低风险内容使用 `log` 记录观察，中风险使用 `replace` 脱敏处理，高风险使用 `block` 直接拦截。
:::
...
:::tip
Webhook 和邮件通知可同时启用。对于高风险策略，建议配置邮件通知以便安全团队及时响应。
:::
...
:::warning
策略删除需要在确认弹窗中**手动输入策略名称**进行二次验证，防止误删。此操作不可撤销。
:::
...
![审查词库](/assets/screenshots/boss/gateway-moderation-lexicon.png)
...
点击 **导入** 按钮打开 词库导入对话框，支持从文件批量导入敏感词条。
...
:::warning
删除词条会立即影响审查策略的匹配行为。如果不确定词条是否仍需保留，建议先降低其得分而非直接删除。
:::
...
:::tip
策略优先级决定了评估顺序。建议将 `block` 类策略设为最高优先级，确保极高风险内容优先被拦截。
:::
...
1. 关注 [审计日志](./audit) 中 `blocked` 结果的记录，分析是否存在误拦截
2. 通过 [运营概览](./operations) 查看被拦截请求的占比趋势
