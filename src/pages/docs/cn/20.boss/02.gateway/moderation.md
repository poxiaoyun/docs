...
title: '内容审查'
updated: '2026-09-14'
description: '带你配好内容审查：维护敏感词库、定义命中后动作，并核对命中结果。'
tags:
  - boss
  - gateway
---
...
![敏感词管理：四张统计卡、分类标签与筛选条，下方是敏感词表格](/assets/screenshots/boss/gateway-lexicon-01.png)

这一页顶部是四张统计卡（词条总数、启用中、本月新增、近 7 日命中），中间是搜索框和 **全部 / 政治 / 暴恐 / 民生 …** 分类标签，再下面是 **所有分类 / 风险等级 / 标签 / 更新时间** 四个筛选下拉。表格里 **敏感词**、**分类**、**标签** 三列已做模糊处理——本页只说明这些列怎么用，具体词条不宜出现在公开文档里。
...
![策略管理列表：策略名称、触发条件、执行动作、优先级、启用状态、更新时间](/assets/screenshots/boss/gateway-policy-01.png)

策略列表的一行就是一条规则：**触发条件** 是一个分数阈值（截图里是 `≥ 50`），**执行动作** 是命中后做什么（截图里是「拦截请求」），**优先级** 数值越小越先匹配。启用与否用 **是否启用** 那一列的开关直接切。
