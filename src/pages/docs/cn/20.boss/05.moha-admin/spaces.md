...
:::tip
Space 类似于 HuggingFace Spaces，是面向用户的模型演示入口。BOSS 端管理员可以全局调度和管理所有 Space 实例。
:::
...
![Space 管理](/assets/screenshots/boss/moha-spaces.png)
...
:::tip
Space 在长时间无访问后可能自动进入 Sleeping 状态以节省资源。用户再次访问时会自动唤醒。
:::
...
![Space 详情](/assets/screenshots/boss/moha-spaces-detail.png)
...
:::warning
停止或删除正在被用户使用的 Space 会立即中断用户的使用体验，建议提前通知用户。
:::
...
:::tip
建议定期巡查 Error 状态的 Space，及时通知用户处理异常，保持平台整体健康。
:::
...
:::tip
普通用户和租户管理员应通过 Console → Moha → Space 来创建和管理自己的 Space 应用。
:::
