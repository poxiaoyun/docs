...
:::tip
此处的镜像仓库指容器镜像（Container Image），用于 AI 训练和推理任务的运行环境，与数据镜像同步（Mirrors）不同。
:::
...
![镜像仓库管理](/assets/screenshots/boss/moha-images.png)
...
页面顶部提供 筛选栏 组件，支持多维度过滤：

- **名称搜索**：按镜像仓库名称模糊搜索
- **租户/组织筛选**：按所属租户或组织过滤
- **可见性筛选**：公开 / 私有

![镜像仓库过滤栏](/assets/screenshots/boss/moha-images-filter.png)
...
![镜像标签列表](/assets/screenshots/boss/moha-images-tags.png)
...
:::tip
安全扫描由平台自动触发，管理员可根据扫描结果决定是否允许该镜像继续使用，或通知用户修复漏洞。
:::
...
:::warning
修改可见性会立即影响其他用户的镜像拉取权限。如有正在运行的容器引用了该镜像，不会受到影响（镜像已缓存），但新的部署请求将受到限制。
:::
...
:::tip
建议定期扫描所有公开镜像的安全漏洞，特别关注 Critical 和 High 级别的漏洞。
:::
...
:::tip
普通用户和租户管理员应通过 Console → Moha → 镜像仓库 来管理自己的容器镜像。
:::
