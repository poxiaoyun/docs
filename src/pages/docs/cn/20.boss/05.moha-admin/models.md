...
:::tip
BOSS 模型库管理是平台运营的核心入口。Console 端用户只能看到自己有权限的模型，而 BOSS 端管理员可以看到全平台所有模型。
:::
...
![模型库管理](/assets/screenshots/boss/moha-models.png)
...
![过滤栏](/assets/screenshots/boss/moha-models-filter.png)
...
:::warning
镜像标签表示该模型是通过镜像同步功能从外部平台（如 HuggingFace、ModelScope）同步而来的，此类模型的编辑可能受到限制。
:::
...
![编辑模型](/assets/screenshots/boss/moha-models-edit.png)
...
:::warning
将公开模型设为私有后，已经引用该模型的其他用户可能会受到影响，请谨慎操作。
:::
...
:::tip
普通用户和租户管理员应通过 Console → Moha → 模型 来管理自己的模型仓库。
:::
