...
:::tip
TOTP 是行业标准的双因素认证方案，被 Google、GitHub、AWS 等主流平台广泛采用，安全可靠。
:::
...
:::warning
当平台启用「强制 MFA」策略后，所有用户必须绑定 MFA 才能正常使用平台。请提前准备好 Authenticator 应用。
:::
...
:::tip
推荐使用支持云端备份的应用（如 Microsoft Authenticator 或 Authy），以防设备丢失后无法恢复。如果使用 Google Authenticator，请务必保存恢复代码。
:::
...
![MFA 步骤一：扫描 QR 码](/assets/screenshots/console/mfa-step1.png)

1. 进入 **个人中心** → **安全设置**（或在强制 MFA 场景下自动跳转）
2. 点击 **「启用 MFA」** 按钮
3. 系统调用 `初始化 MFA` 接口，服务端生成 TOTP 密钥
4. 页面显示一个 QR 码，同时在下方显示密钥文本（Secret Key）

界面组成：

| 区域 | 说明 |
|------|------|
| QR 码图像 | 二维码图片，包含 TOTP 密钥信息 |
| 密钥文本 | QR 码中包含的密钥明文，点击可复制，用于手动输入 |
| 操作提示 | 引导用户使用 Authenticator 应用扫码 |

:::tip
如果您的手机无法扫描 QR 码（例如使用桌面版 Authenticator），可以点击「无法扫码？」链接，然后手动复制页面上显示的密钥文本，在 Authenticator 应用中选择「手动输入」方式添加。
:::
...
![MFA 步骤二：验证验证码](/assets/screenshots/console/mfa-step2.png)

1. 查看 Authenticator 应用中显示的当前 6 位动态验证码
2. 在页面的验证码输入框中输入该 6 位数字
3. 点击 **「验证」** 按钮
4. 系统调用 `验证 MFA` 接口验证输入的验证码
5. 验证通过后，MFA 绑定成功

:::warning
动态验证码每 30 秒刷新一次。如果您输入时验证码刚好切换，请输入最新的验证码。如果连续验证失败，请检查手机的系统时间是否准确——TOTP 验证码的生成严格依赖时间同步。
:::

### 步骤四：保存恢复代码

![MFA 恢复代码](/assets/screenshots/console/mfa-recovery.png)
...
:::warning
**请务必安全保存恢复代码！** 这是设备丢失时恢复账号访问的唯一方式。建议将恢复代码保存在安全的离线位置（如打印出来放在保险箱中），不要保存在手机上。
:::
...
![MFA 登录验证](/assets/screenshots/console/mfa-login.png)
...
:::warning
每个恢复代码只能使用一次。使用后该代码自动失效。当恢复代码剩余数量较少时，建议尽快重新绑定 MFA 或生成新的恢复代码。
:::
...
:::warning
如果平台启用了「强制 MFA」策略，您将无法禁用 MFA。禁用按钮会显示为灰色不可点击状态，并提示「平台要求所有用户启用 MFA」。
:::
...
:::tip
管理员重置 MFA 时可能需要您提供身份验证（如当面确认、邮箱验证等），这是为了防止他人冒用您的身份申请重置。
:::
