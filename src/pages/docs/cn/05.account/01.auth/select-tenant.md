...
:::tip
租户间的资源完全隔离。您在租户 A 中创建的实例、数据集、模型等资源，在租户 B 中不可见，也不可访问。
:::
...
![租户选择页面](/assets/screenshots/console/select-tenant.png)
...
1. 登录成功后，系统自动调用 `获取租户列表` 获取您的租户列表
2. 页面以卡片列表形式展示您有权访问的所有租户
3. 浏览租户列表，确认各租户中您的角色
4. 点击目标租户卡片即可进入该租户的控制台

:::tip
如果您只属于一个租户，登录后会自动跳过此页面，直接进入该租户的控制台首页。
:::
...
![注册新租户](/assets/screenshots/console/register-tenant.png)
...
:::warning
租户 ID 创建后不可更改，它会出现在 API 调用和资源标识中，请使用有意义的简短英文名称。例如：`ai-research-team`、`product-dept`。
:::
...
:::tip
新创建的租户默认没有资源配额。在手动审批模式下，管理员会在审批时分配初始配额；在自动审批模式下，系统会分配预设的默认配额。如果您需要更多资源，请联系系统管理员调整配额。
:::
...
![切换租户](/assets/screenshots/console/switch-tenant.png)
...
:::warning
切换租户后，之前在旧租户中打开的页面可能会失效。如果您正在进行操作（如创建实例），请先完成或取消操作再切换租户。
:::
...
:::tip
如果您在某个租户中需要更高的权限，请联系该租户的管理员为您调整角色。
:::
