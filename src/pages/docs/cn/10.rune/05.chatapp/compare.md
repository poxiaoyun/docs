...
:::tip
参数弹窗设计方式使页面整洁，当需要调参时可以快速打开修改而不遮挡对话内容。
:::
...
:::warning
如果某个模型不支持深度思考功能，开启后该侧可能不会产生思考内容，但不会报错。
:::
...
:::tip
对比时建议每次只改变一个变量（模型或参数），这样能更准确地判断差异来源。
:::
...
:::warning
对比页面的对话历史在页面刷新后会清除。建议对重要的对比结果进行截图或记录。
:::
