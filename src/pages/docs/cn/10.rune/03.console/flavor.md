...
description: 计算资源规格的查看、选择与自定义配置。
tags:
 - rune
 - console
---
...
:::tip
规格由平台管理员在 BOSS 端创建和管理。Console 端的用户只能查看和选择可用的规格，不能创建或修改规格。
:::
...
:::tip
在推理服务、微调任务等实例列表中，Flavor 列统一使用这种可读格式展示，帮助用户快速了解每个实例占用的资源量。
:::
...
:::warning
不同集群安装的加速器类型可能不同。在选择规格前，请确认目标集群支持所需的加速器类型。
:::
...
:::tip
部署实例时，系统自动调用工作空间级的规格列表接口，只显示当前工作空间有配额、且集群中未售罄的规格。
:::
...
:::warning
如果所有 GPU 规格均已售罄，可能需要等待其他实例释放资源，或联系管理员增加集群硬件。
:::
