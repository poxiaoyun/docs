...
description: 模型微调任务的创建、参数配置、训练监控与结果导出。
tags:
 - rune
 - console
---
...
微调服务属于 Instance 架构中 类别，与推理服务共享相同的底层实例模型和部署机制。微调任务的所有超参数（学习率、批次大小、训练轮次、基础模型、训练数据集等）均通过 Helm Chart 的 Schema 定义，在 表单 中动态渲染。

### 核心能力

- **模板驱动**：基于 Helm Chart 模板定义微调工具链（如 LLaMA-Factory、Swift 等），所有参数通过 表单 配置
...
微调任务列表中提供 **Web 访问** 按钮（地址选择），用于通过浏览器访问训练工具的 Web UI，例如 LLaMA-Factory 的可视化训练界面。

:::tip
Web 访问按钮仅在实例状态为 Healthy 时可用。按钮根据端点 URL 优先级自动选择最佳访问地址。
:::
...
微调模板的所有超参数均定义在 Helm Chart 的  中，通过 表单 动态渲染。支持**图形化模式**和 **JSON 编辑模式**两种方式。
...
:::warning
不同模板的超参数可能不同。以上仅为常见参数示例，实际可配置的参数由所选模板的 Schema 定义决定。
:::
...
:::tip
训练数据的格式要求取决于所选的微调模板。例如，LLaMA-Factory 支持 Alpaca 和 ShareGPT 两种数据格式，请参阅模板文档了解详情。
:::
...
:::tip
与推理服务不同，微调任务是 **一次性任务**。训练完成后状态变为 Succeeded，不支持"停止/启动"循环。如需重新训练，需要创建新的微调任务实例。
:::
...
展示实例的 服务信息 和 实例列表，与推理服务的概览页结构一致。
