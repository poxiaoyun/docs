...
日志（Logging）是 Rune 平台中用于查看和分析工作负载运行日志的核心可观测性功能。平台提供了功能丰富的日志查看器（日志查看器），支持**历史日志查询**（基于 HTTP GET，采用 LogQL 语法）和**实时日志流**（基于 WebSocket 推送），覆盖实例级、工作空间级和集群级三个层级的日志查看需求。
...
| 标签过滤 | 实时流同样支持标签过滤 |
| 断开提示 | 连接断开时按关闭码给出提示（正常关闭不提示），需手动重新开启实时流 |
...
> ⚠️ 注意: 日志系统有存储时效限制，超过保留期限的历史日志会被清理；具体保留期限取决于平台配置，文档暂未确认。
