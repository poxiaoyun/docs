...
description: 存储卷的创建、挂载、数据管理与生命周期配置。
tags:
 - rune
 - console
---
...
:::tip
名称必须符合 Kubernetes 命名规范 `[a-z0-9]([-a-z0-9]*[a-z0-9])?`，不能以连字符开头或结尾，且在同一命名空间内不能重复。
:::
...
:::tip
卷类型仅作为分类标签用于管理和组织，不会影响存储卷的实际行为。但在部署推理/微调实例时，模板可能会根据卷类型进行筛选推荐。
:::
...
:::warning
只读标志通过 Label 存储在 StorageVolume 资源上。将已有读写卷切换为只读时，已挂载的实例可能需要重启才能生效。
:::
...
:::warning
仅**受管存储卷**（托管在平台存储集群上的卷）支持文件管理器功能。外部挂载的存储卷不提供此功能。
:::
...
:::warning
文件删除操作不可恢复，请谨慎操作。建议在删除前先下载备份重要文件。
:::
...
:::warning
容量变更仅支持**扩容**操作。由于底层存储机制限制，已分配的空间无法缩小。请在创建时合理评估所需容量。
:::
...
:::tip
S3 账户可用于在外部工具（如 `s3cmd`、`rclone`、`boto3`）中直接访问存储卷文件，适合需要批量操作或自动化的场景。
:::
...
:::tip
建议为每个重要的存储卷编写 README.md，描述存储的模型或数据集的来源、版本、格式和使用说明，方便团队协作。
:::
...
:::tip
从 HuggingFace 下载门控模型（如 Llama 系列）需要提供有权限的 API Token。请先在 HuggingFace 官网同意模型许可协议并生成 Access Token。
:::

:::warning
大型模型文件（如 70B 参数模型可能超过 140GB）的下载耗时较长。请确保存储卷容量充足，并遵循网络带宽的限制。任务运行期间请勿删除存储卷。
:::
...
:::tip
当使用率超过 80% 时，建议及时扩容或清理不再需要的文件，避免写入失败。
:::
