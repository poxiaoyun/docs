...
description: 推理服务的部署、版本管理、流量配置与监控。
tags:
 - rune
 - console
---
...
推理服务是 Rune 平台的核心功能之一，用于将已训练好的 AI 模型部署为在线推理（预测）服务。平台采用基于 Helm Chart 的模板驱动式部署架构，通过 表单 动态渲染部署表单，支持 vLLM、OpenAI 兼容接口等多种推理引擎，部署后可获得标准化的 API 端点供业务系统调用。
...
:::warning
ID 字段一旦创建后不可修改，且在同一命名空间内必须唯一。请使用有意义的命名，例如 `llama3-70b-vllm`。
:::

### 步骤二：配置模板参数（表单）

模板参数通过 **表单** 组件动态渲染。每个推理模板的 Helm Chart 中包含 ，定义了该模板所需的全部可配置参数。表单 支持两种编辑模式：
...
![表单 图形模式](/screenshots/console/inference-schema-form.png)

#### JSON 编辑模式

点击编辑器右上角的切换按钮可进入 JSON 原始编辑模式，适合高级用户直接修改 `values.yaml` 内容。

![表单 JSON 模式](/screenshots/console/inference-schema-json.png)

:::tip
两种模式的数据是实时同步的。在图形模式中修改参数后切换到 JSON 模式可检查实际值，反之亦然。
:::
...
:::tip
停止操作会释放 GPU 等计算资源但保留 Helm Release 配置。当资源紧张时可暂时停止不常用的推理服务，需要时随时恢复。
:::
...
:::warning
停止和启动操作不会改变实例的配置参数，但会导致服务短暂不可用。请确保在业务低峰期进行操作。
:::
...
- 显示名称、描述
- 表单 中的模板参数（如副本数、模型路径等）
- 挂载的存储卷

:::warning
实例 ID 和所使用的模板不可修改。修改部分参数（如 GPU 数量）可能需要重启实例才能生效。
:::
...
:::warning
删除操作不可恢复。挂载的存储卷不会被删除，其中的模型文件仍然保留。
:::
...
:::tip
监控页面的数据来源于集群中部署的 Prometheus 实例。首次加载可能需要几秒钟。支持自定义时间范围查询。
:::

### 日志（Logging）

日志查看器（日志查看器）显示 Pod 容器的实时日志输出：
...
:::tip
当实例状态为 Failed 时，事件页面通常包含最有价值的错误信息，是排查问题的首选入口。
:::
