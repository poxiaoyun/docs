...
- **DCU** 是海光的 AI 加速卡，全称深度计算单元（Deep Computing Unit）：面向 AI 训练与推理的并行计算加速卡，靠大量计算单元同时做矩阵运算。它和通用 GPU 都做并行计算，区别在定位——DCU 面向 AI 负载，通用 GPU 要兼顾图形等更多场景。可以理解成机器里一块「AI 专用显卡」。
...
| 名词 | 通俗解释 |
| --- | --- |
| DCU | 海光的 AI 加速卡（深度计算单元），面向 AI 训练与推理的并行计算加速卡 |
