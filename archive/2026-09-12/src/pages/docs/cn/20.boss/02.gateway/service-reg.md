...
注册完成后，推理服务会自动在网关中创建对应的 [渠道（Channel）](./channels)，从而被纳入网关的路由、负载均衡、限流、审计和内容审查体系中。

:::tip
推理服务注册简化了「部署推理服务 → 注册到网关 → 对外提供 API」的流程，用户无需手动配置渠道参数，系统会自动完成关联。
:::
...
![推理服务注册列表](/assets/screenshots/boss/gateway-service-reg-list.png)
...
:::tip
服务端点通常是 Kubernetes Service 的集群内地址（如 `http://my-inference-svc.namespace.svc.cluster.local:8080/v1`），由系统在部署推理服务时自动生成。
:::
...
:::warning
删除注册不会删除底层的推理服务实例。如需同时清理，请在 Console 中也删除对应的推理服务。
:::
...
:::tip
注册创建的渠道也会出现在 [渠道管理](./channels) 列表中，管理员可以在渠道管理页面进一步调整其路由优先级、限流参数等配置。
:::
